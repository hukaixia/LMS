...
<div class="container col-md-10 col-md-offset-3" style="padding-top:90px">
 <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1" style="width:900px">
    <form class="navbar-form navbar-left" >
         <div class="row">
            <input type="text" style="width:550px;height:45px" name="searchContent" id="hideSearch" class="form-control" placeholder="搜索"/>
			...
			<label><input type="radio" name="searchField" value="bookName"  />  &nbsp;书名 &nbsp; &nbsp;</label>	
			<label><input type="radio" name="searchField" value="bookId"  /> &nbsp;图书ID &nbsp; &nbsp;</label>
         </div>          
    </form>
   </div>
   </br>
					...
					<div class="modal-dialog" role="document">
						<div class="modal-content">
							<div class="modal-header">
								...
								<h4 class="modal-title">图书到期时间更改</h4>
							</div>
							<div class="modal-body">
								 <form action="${pageContext.request.contextPath}/admin/updateBorrow" method ="post">
									 <input type="hidden" name="borrowDate" class="borrowDate"/>
									 <input type="hidden" name="bookId" class="bookId"/>
									 <input type="hidden" name="readerId" class="readerId"/>
...
<script>
 ...
 $(function () {
	...
	function to_page(pn) {
	    $.ajax({
	        url:"${pageContext.request.contextPath}/getBorrowList",
	        data:{"pn":pn,
	...
	function build_reader_table(result) {
	    ...
	    $.each(reader,function (index, item) {
	        ...
	        editBtn.click(function(){
	        	editorBook(item.readerId,item.borrowDate,item.bookId,result.objectMap.mylist.pageNum);
	        });
	...
	function delBook(readerId,borrowDate,bookId,page){
		  if(confirm("确认删除吗？")){
	   	   $.ajax({
				  url: "${pageContext.request.contextPath}/admin/deleteBorrow",
	              method: "post",
				  data:{
					 "readerId":readerId,
...
function editorBook(readerId,borrowDate,bookId,page){
    $(".bookId").val(bookId);
